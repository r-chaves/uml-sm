...
class TestUMLFigure14_36(unittest.TestCase):
    """
    TestCase for UML Figure 14.36.
    """
    def test_figure_14_36(self):
        """
        Test script for UML 2.5 Figure 14.36.

        Compiles the UML SM grammar, uses it to parse the text file containing
        the DSL definition of the state machine in Figure 14.36 of the UML 2.5
        specification, and then compares the JSON representation to that of
        the expected output.
        """
        with open('uml_sm.ebnf', 'r') as grammar_file:
            grammar = grammar_file.read()
        with open('figure_14_36.txt', 'r') as stm_file:
            stm_desc = stm_file.read()
        with open('figure_14_36_expected.json', 'r') as expected_json_fp:
        ...
        try:
            ...
            with open('figure_14_36_failed.json', 'w') as failed_json_fp:
